...
public:
  ...
  double C_thresh[2][NO_OF_LOCI]; //threshold from Poethke and Hovestdtat (2002)
};
  ...
  for(int x=world_size_x/2-burn_in_x/2;x<world_size_x/2+burn_in_x/2;x++)
  {
    ...
    //Initialise No individuals in each patch
      for(int n=0; n<No; n++)
      {
          ...
          for(int i=0;i<NO_OF_LOCI;i++)
          {
            newind.C_thresh[0][i]=ran();  //initialise genotype with standing genetic variation
            newind.C_thresh[1][i]=ran();
          }
    ...
    for(int x=x_min; x<x_max;x++) //output half the genotypes in a specified region
    {
      for(int y=0; y<world_size_y ;y++)
      {
       for(int f=0;f<world[x][y].females.size();f++)
       {
        if(ran()<0.5){
          ...
          for(int i=0;i<NO_OF_LOCI;i++)
          {
            op5<<mean(world[x][y].females.at(f).C_thresh[0][i],world[x][y].females.at(f).C_thresh[1][i])<<" ";
          }
          op5<<endl;
        }
      }
      for(int m=0;m<world[x][y].males.size();m++)
      {
        if(ran()<0.5){
          ...
          for(int i=0;i<NO_OF_LOCI;i++)
          {
            op5<<mean(world[x][y].males.at(m).C_thresh[0][i],world[x][y].males.at(m).C_thresh[1][i])<<" ";
          }
          op5<<endl;
        }
      }
...
double alleles_one_locus(vector<double> all_alleles)    //returns allelic measures for all alleles at a given locus
{

    //stores the freqyuency of a given allele
  vector<int> count_alleles;
    //store the identity of the allele                           
  vector<double> alleles;       
    //set the first allele
  alleles.push_back(all_alleles.at(0));
    //set the counter of the allele to 1   
  count_alleles.push_back(1);
    //go through all the alleles
  for(int a=0;a<all_alleles.size();a++)
  {
      //this flag says that a match has been found
    bool flag=0;
      //go through all previously stored alleles, and search for previously stored alleles
    for(int c=0; c<count_alleles.size();c++)
    {
      if(all_alleles.at(a)==alleles.at(c))
      {
        count_alleles.at(c)++;
    ...
    if(flag==0)
    {
      alleles.push_back(all_alleles.at(a));
      count_alleles.push_back(1);
    }
    all_alleles.erase(all_alleles.begin()+a);
    a--;
  ...
  for(int c=0;c<count_alleles.size();c++)
  { 
    p_sum=p_sum+double(count_alleles.at(c)/accumulate(count_alleles.begin(),count_alleles.end(),0.0))*double(count_alleles.at(c)/accumulate(count_alleles.begin(),count_alleles.end(),0.0));
...
double heterozygosity(int x_min, int x_max)
{
  ...
  if(all_individuals.size()>no_of_individuals){
  ...
  double p_sum_loci=0.0;
  for(int i=0;i<NO_OF_LOCI;i++)
  {
    vector<double> disp_alleles;
    for(int p=0;p<random_individuals.size();p++)
    {
      disp_alleles.push_back(random_individuals.at(p).C_thresh[0][i]);
      disp_alleles.push_back(random_individuals.at(p).C_thresh[1][i]);
    }
    p_sum_loci=p_sum_loci+alleles_one_locus(disp_alleles);

  }


  return 1.0-p_sum_loci/double(NO_OF_LOCI);
}
else return 10000;
...
vector< vector <double> > trait_calulator(int x_min,int x_max) //takes the boundaries for which 
{
  vector <TInd> all_individuals;//stores all individuals in patches between x_min to x_max 
  for(int x=x_min;x<x_max;x++)
  {
  ...
  vector<vector<double> > quartile;
    ...
    for(int p=0;p<no_of_individuals;p++){
      ...
      for(int i=0; i<NO_OF_LOCI;i++)
      {
        disp=disp+mean(random_individuals.at(p).C_thresh[0][i],random_individuals.at(p).C_thresh[1][i]);
      }
  ...
  for(int x=0;x<world_size_x;x++)   //dispersal loop
  {
    for(int y=0;y<world_size_y ;y++)  //go through all the patches
    {
      ...
      for(int f=0;f<world[x][y].females.size();f++)
      {
        ...
        dispersal_probability=dispersal_probability_calc(world[x][y].females.at(f).C_thresh,population_density*ALPHA/(LAMBDA-1)); 
        //dispersal
      ...
      {
        double dispersal_probability;
        dispersal_probability=dispersal_probability_calc(world[x][y].males.at(m).C_thresh,population_density*ALPHA/(LAMBDA-1));
...
{ 
        ...
        for(int f=0;f<Nf;f++) //go through all the females
        {
          int mate_position=floor(ran()*world[x][y].males.size());  //choose a male to mate with
            ...
            for(int b=0;b<no_of_babies;b++)         //setting parental characteristics
            {
              ...
              {
                if(ran()<0.5)
                  newind.C_thresh[0][i]=mutate(world[x][y].females.at(f).C_thresh[0][i]);
                else newind.C_thresh[0][i]=mutate(world[x][y].females.at(f).C_thresh[1][i]);
                if(ran()<0.5)
                  newind.C_thresh[1][i]=mutate(world[x][y].males.at(mate_position).C_thresh[0][i]);
                else newind.C_thresh[1][i]=mutate(world[x][y].males.at(mate_position).C_thresh[1][i]);
              }
              if(ran()<0.5)
 ...
 int main()
  ...
  //output the population size, sex ratio, measured dispersal at each patch
    ...
    op2<<"d_"<<0<<"_1 "<<"d_"<<0<<"_2 "<<"d_"<<0<<"_3 ";

    op2<<endl;

    op3<<"d_"<<0<<"_1 "<<"d_"<<0<<"_2 "<<"d_"<<0<<"_3 ";

    op3<<endl;
    op4<<"d_"<<0<<"_1 "<<"d_"<<0<<"_2 "<<"d_"<<0<<"_3 ";
    op4<<endl;
  ...
  if(t<=BURN_IN_TIME)
  {
    if(t%2500==0)
    {
      ...
      vector<vector<double> > quartile=trait_calulator(world_size_x/2-burn_in_x/2, world_size_x/2+burn_in_x/2);
      op2<<r<<" "<<t<<" ";
  ...
  if(t>BURN_IN_TIME && t%10==0)
  {
    ...
    op2<<endl;
    quartile=trait_calulator(margin_x_left,margin_x_left+5);

    op3<<quartile.at(0).at(0)<<" "<<quartile.at(0).at(1)<<" "<<quartile.at(0).at(2)<<" ";

    op3<<endl;
    quartile=trait_calulator(margin_x_right-4,margin_x_right+1);
